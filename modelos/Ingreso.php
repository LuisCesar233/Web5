...
Class Ingreso
{
	...
	{
		$sql="SELECT i.idingreso,DATE(i.fecha_hora) as fecha,i.idproveedor,p.nombre as proveedor,u.idusuario,pu.nombre as usuario,i.tipo_comprobante,i.serie_comprobante,i.num_comprobante,i.total_compra,i.impuesto,i.estado FROM ingreso i, persona p, usuario u, persona pu WHERE(u.idusuario=i.idusuario)AND(p.idpersona=i.idproveedor)AND(pu.idpersona=u.idpersona)AND(i.idingreso='$idingreso')";
	...
	{
		$sql="SELECT i.idingreso,DATE(i.fecha_hora) as fecha,i.idproveedor,p.nombre as proveedor,u.idusuario,pu.nombre as usuario,i.tipo_comprobante,i.serie_comprobante,i.num_comprobante,i.total_compra,i.impuesto,i.estado FROM ingreso i, persona p, usuario u, persona pu WHERE(u.idusuario=i.idusuario)AND(p.idpersona=i.idproveedor)AND(pu.idpersona=u.idpersona) ORDER BY i.idingreso desc";
		return ejecutarConsulta($sql);		
	}
